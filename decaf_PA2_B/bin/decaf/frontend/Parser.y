...
%token VOID   BOOL  INT   STRING  CLASS PLUSONE MINUSONE
%token NULL   EXTENDS     THIS     WHILE   FOR REPEAT UNTIL
%token IF     ELSE        RETURN   BREAK   NEW SWITCH CASE DEFAULT
%token PRINT  READ_INTEGER         READ_LINE
%token LITERAL
%token IDENTIFIER	  AND    OR    STATIC  INSTANCEOF
%token LESS_EQUAL   GREATER_EQUAL  EQUAL   NOT_EQUAL DEC INC
%token '+'  '-'  '*'  '/'  '%'  '='  '>'  '<'  '.'
%token ','  ';'  '!'  '('  ')'  '['  ']'  '{'  '}' '?' ':'
...
Expr            :	LValue
                ...
                |	Expr '?' Expr ':' Expr
                	{
                		$$.expr = new Tree.Ternary(Tree.CONOP, $1.expr, $3.expr, $5.expr, $1.loc);
                	}
                ;
...
SwitchStmt      :   SWITCH '(' Expr ')' SwitchBlock
                    {
                        $$.stmt = new Switch($3.expr, (SwitchBlock)$5.stmt, $1.loc);
                    }
                ;

SwitchBlock     :   '{' Cases '}'
                    {
                        $$.stmt = new SwitchBlock($2.slist, $1.loc);
                    }
                ;
...
Case            :   CASE Expr ':' CaseBlock
                    {
                        $$.stmt = new Case($2.expr, (CaseBlock)$4.stmt, $1.loc);
                    }
                ;

DefaultCase     :   DEFAULT ':' CaseBlock
                    {
                        $$.stmt = new DefaultCase((CaseBlock)$3.stmt, $1.loc);
                    }
                ;

CaseBlock       :   StmtList
                    {
                        $$.stmt = new CaseBlock($1.slist, null);
                    }
                ;
