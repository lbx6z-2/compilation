...
public class TypeCheck extends Tree.Visitor {
	...
	public void visitTernary(Tree.Ternary expr) {
		expr.type = checkTernaryOp(expr.left, expr.middle, expr.right, expr.loc);
	}
	...
	public void visitUnary(Tree.Unary expr) {
		...
		if(expr.tag == Tree.NEG){
			...
			if (!(expr.expr instanceof Tree.LValue)){
				issueError(new NotLValueError(expr.getLocation(), "++"));
			}
			if (expr.expr.type.equal(BaseType.ERROR)
					|| expr.expr.type.equal(BaseType.INT)) {
				expr.type = expr.expr.type;
	...
	public void visitSwitch(Tree.Switch switchstmt) {
		...
		if (!switchstmt.expr.type.equal(BaseType.INT)){
			issueError(new IncompatSwitchError(switchstmt.expr.getLocation(),
					switchstmt.expr.type.toString()));
		}
		breaks.add(switchstmt);
		if (switchstmt.sb != null) {
			switchstmt.sb.accept(this);
		}
		breaks.pop();
	}
	
	public void visitSwitchBlock(Tree.SwitchBlock sb) {
		for (Tree cd : sb.cases) {
			cd.accept(this);
		}
	}
    ...
    public void visitCase(Tree.Case cs) {
    	cs.expr.accept(this);
    	if (!cs.expr.type.equal(BaseType.INT)){
    		issueError(new IncompatCaseError(cs.expr.getLocation()));
    	}
    	if (cs.cb != null){
    		cs.cb.accept(this);
    	}
    ...
    public void visitDefaultCase(Tree.DefaultCase dc) {
    	if (dc.cb != null){
    		dc.cb.accept(this);
    	}
    }
    
    public void visitCaseBlock(Tree.CaseBlock cb){
    	for (Tree tree : cb.block) {
			tree.accept(this);
		}
    }
	...
	private Type checkTernaryOp(Tree.Expr left, Tree.Expr middle, Tree.Expr right, Location location) {
		left.accept(this);
		middle.accept(this);
		right.accept(this);
		
		//如果中或右的type=error，返回中间的type。有可能不对
		if(middle.type.equal(BaseType.ERROR) || right.type.equal(BaseType.ERROR)) {
			return BaseType.ERROR;
		}
		
		checkTestExpr(left);
		if (!middle.type.equal(right.type)){
			issueError(new IncompatTerOpError(location, middle.type.toString(), right.type.toString()));
			return BaseType.ERROR;
		}
